...
class DistilledVisionTransformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        ...
        self.pre = nn.Conv2d(in_channels=1, out_channels=3, kernel_size=1)
        ...
        if self.agg == 'gem':
            ...
            self.aggregation = nn.Sequential(L2Norm(), self.gem, Flatten(),
                                            ...
                                            L2Norm())
        self.mac=nn.AdaptiveMaxPool1d(1)
        self.c_attn =  nn.Sequential(L2Norm(dim=1), self.mac)
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            features.append(self.norm(x)) # layernorm
    ...
    def forward(self, x): # X:B C H W
        ...
        x = x.reshape(-1,1,H,W) # separate process by block
        x = self.forward_features(x) # tokenS
        x = x[:, self.level, :, :] #(B, L, 2+30*30, C) L=3 C=384 480/16 MULTI-LEVEL
        x = x.permute(0,2,1,3).reshape(x.size(0), x.size(2), -1) #(B,2+30*30,C*L)

        # channel process per patch
        x = x.reshape(B,C,902,1152)
        x = x.permute(0,2,3,1).reshape(-1,1152,C)
        x = self.c_attn(x)[:,:,0]
        x = x.reshape(B,-1,1152)
        if self.agg=='gem':
        ...
        # CLS+DIS
            ...
            x = self.l2_norm((self.l2_norm(x) + self.l2_norm(x_dist)) / 2)
...
def deit_base_distilled_patch16_384(pretrained=True, img_size=(384,384), num_classes =1000, inputc=1, depth=12, agg='gem', level=[1,3,5], **kwargs):
    model = DistilledVisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=768, depth=depth, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_classes=num_classes, inputc=inputc, agg=agg, level=level, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(

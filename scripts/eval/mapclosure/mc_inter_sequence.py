...
def evaluate_dataset(database_sets, query_sets, log: bool = False,
    ...
    for set in tqdm.tqdm(database_sets, disable=not show_progress, desc='Computing database embeddings'):
        scs, rks = get_latent_vectors(set,mode='DB')
    ...
    for set in tqdm.tqdm(query_sets, disable=not show_progress, desc='Computing query embeddings'):
        scs, rks = get_latent_vectors(set,mode='Q')
    ...
    return stats,None,None #, mrr_grid, recall_1_grid



@torch.no_grad()
def get_latent_vectors(t_set,mode=None):
    ...
    ks, ds = [], []
    Dict={}
    run_name = mode+t_set[0]['query'].split('/')[1]
    datafile = os.path.join(configs.data.evalset_folder, run_name+'_bev_'+'0.5_0.5_100_100_norm'+'.npy')
...
def get_recall(m, n, database_embeddings_sc, database_embeddings_rk, query_embeddings_sc, query_embeddings_rk, query_sets, database_sets, ax, log=False):
    ...
    embeddings_d = database_embeddings_rk[m]
    ...
    num_neighbors = len(embeddings_d)

    recall = [0] * 25
    recall_idx = []

    one_percent_retrieved = 0
    threshold = max(int(round(len(embeddings_d)/100.0)), 1)
    ...
    for query_i in tqdm.tqdm(range(len(queries_output))):
        ...
        descriptors = queries_output[query_i]
        ...
        # reranking
        num_neighbor = min(25,num_neighbors)
        sc_dist = np.zeros((num_neighbor,))
        for nn_i,candidate_ndx in enumerate(nn_ndx[:num_neighbor]):
            query_sc = queries_output[query_i]
            candidate_sc = embeddings_d[candidate_ndx]
            matches = match_features(candidate_sc, query_sc)
            H,inliers = ransac_homography(database_scs[candidate_ndx],query_scs[query_i],matches)
            if inliers >= 10:

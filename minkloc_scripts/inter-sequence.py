...
import torch
from minkloc_utils import get_latent_vectors
from scripts.util import load_from_pickle
    ...
    for query_sets, database_sets, query_feat, database_feat, location in zip(queries, databases, query_features, database_features, args.location_names):
        ...
        if query_feat == 'None' or database_feat == 'None':
            ...
            for i in range(len(query_sets)):
                database_feat.append(get_latent_vectors(model, database_sets[i], args.root[0], 'cuda',params))
                query_feat.append(get_latent_vectors(model, query_sets[i], args.root[0], 'cuda',params))
